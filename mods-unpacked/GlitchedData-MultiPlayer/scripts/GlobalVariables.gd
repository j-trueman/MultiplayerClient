...
func _ready():
	if using_steam: currentVersion = currentVersion_nr + "." + str(currentVersion_hotfix) + versuffix_steam
	else: currentVersion = currentVersion_nr + "." + str(currentVersion_hotfix) + versuffix_itch
...
func get_current_scene_node() -> Node:
	return get_tree().get_root().get_child(len(get_tree().get_root().get_children())-1)