...
const ButtonClass = preload("res://mods-unpacked/GlitchedData-MultiPlayer/scripts/ButtonClass.gd")
...
const OptionsManager = preload("res://mods-unpacked/GlitchedData-MultiPlayer/scripts/OptionsManager.gd")
...
@export var parent_mods : Control
...
func _input(event):
	...
	if (event.is_action_pressed("ui_cancel") && failsafed and not viewingConfig):
	...
	if (event.is_action_pressed("exit game") && !fs2 && viewing_intro):
		SkipIntro()
		fs2 = true
	if (event.is_action_pressed("ui_accept") && failsafed and viewingConfig):
		ReturnToLastScreen()
...
@export var firstFocus_mods : Control
...
func ReturnToLastScreen():
	...
	if (currentScreen == "mods" and parent_modConfig.has_node("mod config")):
		parent_modConfig.get_node("mod config").queue_free()
		parent_modConfig.get_node("Title/Title").text = "List of installed mods"
		parent_modConfig.get_node("ScrollContainer").visible = true
		currentScreen = "main"
		lastScreen = "mods"
	Show(lastScreen)
	ResetButtons()
...
func ModConfigMenu(menu:String,mod_name:String):
	var loadedmenu = load(menu).instantiate()
	loadedmenu.name = "mod config"
	parent_modConfig.get_node("ScrollContainer").visible = false
	parent_modConfig.get_node("Title/Title").text = mod_name + " config"
	parent_modConfig.add_child(loadedmenu)
